...
class my_Load(Dataset):
    ...
    def cache_labels(self, path=Path('./labels.cache'), prefix=''):
        ...
        try:
            ...
            logging.info(
                f'{prefix}WARNING: Cache directory {path.parent} is not writeable: {e}')  # path not writeable
        return x
    ...
    def collate_fn4(batch):
        ...
        for i in range(n):  # zidane torch.zeros(16,3,720,1280)  # BCHW
            ...
            if random.random() < 0.5:
                im = \
                F.interpolate(img[i].unsqueeze(0).float(), scale_factor=2., mode='bilinear', align_corners=False)[
                    0].type(img[i].type())
                l = label[i]
            else:
