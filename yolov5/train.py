...
from models.yolo_ import Model
...
from utils.plots_ import plot_labels, plot_evolve
...
def train(hyp,  # path/to/hyp.yaml or hyp dictionary
    ...
    if pretrained:
        ...
        # ckpt.pop('model.24.m.0.weight')
        # ckpt.pop('model.24.m.0.bias')
        # ckpt.pop('model.24.m.1.weight')
        # ckpt.pop('model.24.m.1.bias')
        # ckpt.pop('model.24.m.2.weight')
        # ckpt.pop('model.24.m.2.bias')
...
def parse_opt(known=False):
    ...
    #    无迁移学习
    opt.weights = ''
    opt.hyp = 'data/hyps/hyp_evolve.yaml'
    opt.cfg = 'models/yolov5s.yaml'
    # if weights = yolov5s.pt
    #    opt.cfg = ''
    #    迁移学习yolov5s.pt

    opt.data = r'data\v4v5_dataset.yaml'
    ## 设置 cfg 从头开始 设置weights 迁移学习 data里面将yaml的nc改为需要的nc
    opt.epochs = 500
    opt.batch_size = 10
    opt.imgsz = 640
    opt.name = 'train'
    opt.project = 'v4v5runs/913train'
    opt.change_nc = False #24-27
    opt.restart = False
    return opt
...
def main(opt):
    ...
    # Train
    if not opt.evolve: # 如果不evlove 会使用hyp选择的一些之前的经验参数
