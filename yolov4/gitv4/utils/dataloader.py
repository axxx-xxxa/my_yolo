...
class YoloDataset(Dataset):
    ...
    def get_random_data(self, annotation_line, input_shape, jitter=.3, hue=.1, sat=1.5, val=1.5, random=True):
        ...
        if '.jpg' in annotation_line:
            img_path = annotation_line.split(".jpg",1)[0]+'.jpg'
            line = copy.deepcopy(annotation_line.split(".jpg", 1)[1].split())
        else:
            img_path = annotation_line.split(".png", 1)[0] + '.png'
            line = copy.deepcopy(annotation_line.split(".png", 1)[1].split())
        ...
        scale = self.rand(.25, 2) # 0.25-2.0 缩放倍数范围
        if new_ar < 1:
            nh = int(scale * h)
            nw = int(nh * new_ar)
        else:
    ...
    def get_random_data_with_Mosaic(self, annotation_line, input_shape, hue=.1, sat=1.5, val=1.5):
        ...
        for line in annotation_line:
            # 每一行进行分割
            if '.jpg' in line:
                img_path = line.split(".jpg", 1)[0] + '.jpg'
                boxes = line.split(".jpg",1)[1]
                index_ = line.index(' ')
                index_img = line.index('.jpg')
            else:
                img_path = line.split(".png", 1)[0] + '.png'
                boxes = line.split(".png", 1)[1]
                index_ = line.index(' ')
                index_img = line.index('.png')
            line_content = img_path + boxes
            line_content = line.split()
            if index_ < index_img:
                line_content[0] = ' '.join(line_content[:1])
                del line_content[1]
            # print(line_content)
            # 打开图片
