...
act_class = ['head' ,'person' , 'cellphone', 'lamp']
def Read_Write_classestxt_trainanno_(xml_root, img_root):
    ...
    for i, xml_name in enumerate(xml_list):
        ...
        # root = tree.getroot()
        if 'jpg' in tree.findtext('./filename'):
            img_file_path = tree.findtext('./filename')
        else:
            img_file_path = tree.findtext('./filename').replace('png','jpg')
...
if __name__=="__main__":
    xml_roots = ['../dataset_for_v4v5/label']
    img_roots = ['../dataset_for_v4v5/images']
